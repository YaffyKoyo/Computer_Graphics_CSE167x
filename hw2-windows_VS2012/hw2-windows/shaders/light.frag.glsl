...
vec4 ComputeLight (const in vec3 direction, const in vec4 lightcolor, const in vec3 normal, const in vec3 half_vec, const in vec4 mydiffuse, const in vec4 myspecular, const in float myshininess) {
	...
	if (enablelighting) {       
		...
		for(int i = 0; i < numused; i++){
			...
			if(lightposn[i].w == 0){
				...
				vec4 col = ComputeLight(direction, lightcolor[i], normal, half_, diffuse, specular, shininess) ;
				finalcolor += col;
			}
			//point
			else{
